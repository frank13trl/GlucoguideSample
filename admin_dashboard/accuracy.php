...
if (mysqli_connect_error()) {
    ...
    if (isset($_POST['userid'])) {
        ...
        $row = mysqli_fetch_row($result);
        $pcount = $row[0];
        $result = mysqli_query($handle, "SELECT MAX(ABS(reading_avg - pricked)), 
                                                MIN(ABS(reading_avg - pricked)), 
                                                AVG(ABS(reading_avg-pricked)) FROM patient_reading WHERE patient_id='$id' and pricked != 0;");
        $row = mysqli_fetch_row($result);
        $maxv = $row[0];
        $minv = $row[1];
        $avgv = $row[2];
        $result = mysqli_query($handle, "SELECT MAX((ABS(reading_avg-pricked)/pricked)*100), 
                                                MIN((ABS(reading_avg-pricked)/pricked)*100), 
                                                AVG((ABS(reading_avg-pricked)/pricked)*100) FROM `patient_reading` WHERE patient_id='$id' and pricked!=0;");
        $row = mysqli_fetch_row($result);
        $maxerr = $row[0];
        $minerr = $row[1];
        $avgerr = $row[2];
